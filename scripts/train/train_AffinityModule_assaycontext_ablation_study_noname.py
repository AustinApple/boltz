"""Leaf-field ablation study with protein/ligand names always excluded (test-time ablation).

Same machinery as ``train_AffinityModule_assaycontext_ablation_study.py``, but here the
protein-name and ligand-name leaf fields are **never** part of the assay-context embedding —
not in the baseline and not in any variant. This isolates the contribution of the remaining
assay-context fields without the (often dominant) protein / ligand identity strings.

Concretely, per assay type a fixed set of "name" leaves is dropped from every embedding:

    itc: protein.name, ligand.name
    spr: analyte.description, immobilization.ligand_name
    fpa: biological_preparation.target_protein, test_compound.description
    rba: biological_preparation.target_protein, test_compound.description

The **baseline** keeps all fields except those name leaves. Each **variant** removes the name
leaves *plus* one additional leaf field (or the ``assay_type`` tag), so its delta-vs-baseline
measures that one field's importance on top of an already name-free context.

A small head is trained on the pre-computed ``g`` + the name-free baseline embedding once per
seed; for every variant the masked test-set embeddings are fed into that pre-trained baseline
head and the change in test metrics is recorded. No re-training per variant — only the
test-time embeddings differ. The pairformer backbone is **never trained** here; ``g`` comes
from the cached ``<target_dir>/processed/g/<rid>/g_<rid>.npz`` files.

Because the baseline here is itself a masked embedding (names removed), it is NOT read from the
existing ``qwen3/`` dir; every variant — baseline included — is its own ``ablation_<...>/`` dir.

Example:
    python scripts/train/train_AffinityModule_assaycontext_ablation_study_noname.py \
...
# Per-assay protein/ligand "name" leaves that are excluded from every embedding (baseline
# and all variants). These are removed up front and never ablated on their own.
EXCLUDED_NAME_FIELDS: dict[str, list[str]] = {
    "itc": ["protein.name", "ligand.name"],
    "spr": ["analyte.description", "immobilization.ligand_name"],
    "fpa": ["biological_preparation.target_protein", "test_compound.description"],
    "rba": ["biological_preparation.target_protein", "test_compound.description"],
...
# Label used in the summary for the name-free, nothing-else-removed embedding.
BASELINE_LABEL = "baseline"


def build_variants(assay_type: str, schema_path: Path) -> list[tuple[str, str]]:
    """Return ``(label, embed_variant)`` pairs for the name-free ablation study.

    - ``label`` is what appears in the summary (``baseline`` / a leaf path / ``assay_type``):
      which *additional* field was removed on top of the always-excluded name fields.
    - ``embed_variant`` is the ``+``-joined string fed to ``ablation_embed`` / used for the
      cache dir, i.e. the full set of leaves to drop for that embedding.

    Baseline drops only the name fields; every other variant drops the name fields plus one
    more leaf (or the ``assay_type`` tag).
    """
    excluded = EXCLUDED_NAME_FIELDS[assay_type]
    base_variant = "+".join(excluded)

    leaf_fields = enumerate_leaf_fields(schema_path)
    other_fields = [f for f in leaf_fields if f not in excluded]

    pairs: list[tuple[str, str]] = [(BASELINE_LABEL, base_variant)]
    for f in other_fields:
        pairs.append((f, "+".join([*excluded, f])))
    pairs.append((ASSAY_TYPE_VARIANT, "+".join([*excluded, ASSAY_TYPE_VARIANT])))
    return pairs


def embedding_model_for(embed_variant: str) -> str:
    """The ``assay_embedding_model`` subdir an embed-variant reads from.

    Every variant here (baseline included) is a masked embedding, so all read from their own
    ``ablation_<dirname>/`` dir — none reuse the unmasked ``qwen3/`` embeddings.
    """
    return f"ablation_{variant_dirname(embed_variant)}"
...
def train_baseline(
    *,
    ...
    """Train the name-free baseline head once per seed; return the checkpoint root dir."""
    ckpt_root = Path(target_dir) / "checkpoints" / f"ablation_noname_{assay_type}" / "baseline"
    assay_embedding_model = embedding_model_for(baseline_variant)
    ...
    for seed in seed_list:
        ...
        if use_wandb:
            wandb_logger = WandbLogger(
                ...
                entity=wandb_entity,
                name=f"{assay_type}_noname_baseline_seed{seed}",
                group=f"ablation_noname_{assay_type}",
...
def run_variant(
    ...
    *,
    ...
    The head weights come from the name-free baseline checkpoint; only the test-time
    ``assay_embedding_model`` differs across variants. No re-training.
    """
    assay_embedding_model = embedding_model_for(embed_variant)

    mses, rps, cindexes = [], [], []
    for seed in seed_list:
        ckpt_path = baseline_ckpt_root / f"seed_{seed}.ckpt"
        if not ckpt_path.exists():
            print(f"[{label}] seed {seed}: baseline checkpoint missing at {ckpt_path}, skipping")
            continue
        datacfg = _datacfg(
            assay_type=assay_type,
            df_path=df_path,
...
def write_summary(rows: list[dict], assay_type: str) -> None:
    ...
    baseline = df[df["variant"] == BASELINE_LABEL]
    ...
    df["_order"] = (df["variant"] != BASELINE_LABEL).astype(int)
    df = df.sort_values(["_order", "delta_rp_vs_baseline"]).drop(columns="_order").reset_index(drop=True)

    out_csv = _THIS_DIR / f"ablation_results_noname_{assay_type}.csv"
    out_md = _THIS_DIR / f"ablation_results_noname_{assay_type}.md"
    df.to_csv(out_csv, index=False)

    excluded = ", ".join(EXCLUDED_NAME_FIELDS[assay_type])
    cols = ["variant", "n_seeds", "mse_mean", "mse_std", "rp_mean", "rp_std",
            "cindex_mean", "cindex_std", "delta_rp_vs_baseline"]
    lines = [
        f"# Ablation results (name-free) — {assay_type.upper()}",
        "",
        f"Protein/ligand name fields excluded from every embedding: `{excluded}`.",
        "",
        f"Baseline test rp = {base_rp:.4f} (names removed, nothing else). A more negative "
        "`delta_rp_vs_baseline` means removing that field hurt the model more, i.e. the field "
        "is more important on top of an already name-free context.",
        "",
        "| " + " | ".join(cols) + " |",
        "| " + " | ".join(["---"] * len(cols)) + " |",
    ]
...
@click.option("--ablation_fields", type=str, default=None,
              help="Comma-separated subset of variant labels to run (default: baseline + all "
                   "non-name leaf fields + assay_type). 'baseline' is always included.")
...
def main(
    ...
    """Run the name-free leaf-field ablation study for one assay type (head-only training)."""
    ...
    # ---- Step 0: build (label, embed_variant) pairs with names always excluded ----
    full_pairs = build_variants(assay_type, paths["schema"])
    ...
    if requested is None:
        ...
        pairs = [(label, ev) for (label, ev) in full_pairs
                 if label == BASELINE_LABEL or label in requested]

    labels = [label for label, _ in pairs]
    baseline_variant = next(ev for label, ev in pairs if label == BASELINE_LABEL)

    # ---- Step 1: cache embeddings for every variant (baseline included; all are masked) ----
    if not skip_embedding:
        to_embed = [ev for _, ev in pairs]
        cmd = [
    ...
    print(f"[ablation] {assay_type}: {len(pairs)} variants to evaluate: {labels}")
    print(f"[ablation] excluded name fields: {EXCLUDED_NAME_FIELDS[assay_type]}")
    print(f"[ablation] baseline embed variant: '{baseline_variant}'")

    # ---- Step 2: train the name-free baseline head once per seed ----
    ...
    rows = []
    for label, embed_variant in pairs:
        print(f"\n{'=' * 70}\n[ablation] variant: {label}  (embed: {embed_variant})\n{'=' * 70}")

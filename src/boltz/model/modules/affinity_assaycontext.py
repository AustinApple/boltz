...
class AffinityAssayContextModule_concatenate_g_after_meanpooling(nn.Module):
    """Algorithm 31"""
    ...
    def __init__(
        ...
        self.affinity_heads = AffinityHeadsTransformer_concatenate_g_after_meanpooling(
            token_z,
    ...
    def forward(
        ...
        # affinity heads
        out_dict.update(
            self.affinity_heads(z=z, feats=feats, assay_context=assay_context, multiplicity=multiplicity)
        )

        return out_dict


class AffinityHeadsTransformer_concatenate_g_after_meanpooling(nn.Module):
    def __init__(
        self,
        token_z,
        input_token_s,
        ...
        self.affinity_out_mlp = nn.Sequential(
            nn.Linear(token_z + assay_context_dim, token_z + assay_context_dim),
            nn.ReLU(),
            nn.Linear(token_z + assay_context_dim, input_token_s),
            nn.ReLU(),
        )
    ...
    def forward(
        ...
        # concatenate assay context if provided
        if assay_context is not None:
            g = torch.cat([g, assay_context], dim=-1)
...
class AffinityAssayContextModule_dual_film(nn.Module):
    """Algorithm 31"""
    ...
    def __init__(
        ...
        self.assay_context_dim_to_2z_early = LinearNoBias(assay_context_dim, 2 * token_z)
        ...
        self.pairformer_stack = PairformerNoSeqModule(token_z, **pairformer_args)
        self.affinity_heads = AffinityHeadsTransformer_dual_film(
            token_z,
            transformer_args["token_s"],
            assay_context_dim,
            transformer_args["num_blocks"],
    ...
    def forward(
        ...
        gamma, beta = self.assay_context_dim_to_2z_early(assay_context).chunk(2, dim=-1)
        z = gamma[:, None, None, :] * z + beta[:, None, None, :]
...
class AffinityHeadsTransformer_dual_film(nn.Module):
    def __init__(
        self,
        token_z,
        input_token_s,
        ...
        self.assay_context_dim_to_2z_late = LinearNoBias(assay_context_dim, 2 * token_z)
    ...
    def forward(
        ...
        gamma, beta = self.assay_context_dim_to_2z_late(assay_context).chunk(2, dim=-1)
        g = gamma * g + beta

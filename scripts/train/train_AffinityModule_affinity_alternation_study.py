...
from boltz.data.module.training_AffinityModule_affinity_alternation_study import AffinityModuleDataModule, DataConfig
...
@click.option("--split_method", type=click.Choice([
    ...
    'protein_seqid',
    'wt_mutation'
]), default='wt_mutation', show_default=True, help='Data splitting strategy for train/val/test.')
...
def main(
    ...
    # Evaluation phase
    for seed in range(num_seeds):
        if os.path.exists(f'Boltz2_wt_mutation_seed_{seed}_results.npz'):
            print(f"Results for seed {seed} already exists, skipping evaluation.")
            continue
        Datacfg = DataConfig(
            df_path=df_path,
            split_method=split_method,
        ...
        train_loader = AffinityModuleDataModule(Datacfg).train_dataloader()
        valid_loader = AffinityModuleDataModule(Datacfg).val_dataloader()
        test_loader = AffinityModuleDataModule(Datacfg).test_dataloader()[0]
        trainer = Trainer(devices=[int(device)], accelerator="gpu", log_every_n_steps=1)
        train_mse, train_rmse, train_rp, train_cindex, train_pred, train_label = val(model_module, train_loader, trainer)
        valid_mse, valid_rmse, valid_rp, valid_cindex, valid_pred, valid_label = val(model_module, valid_loader, trainer)
        test_mse, test_rmse, test_rp, test_cindex, test_pred, test_label = val(model_module, test_loader, trainer)

        np.savez(f'Boltz2_wt_mutation_seed_{seed}_results.npz',
            train_pred=train_pred.numpy(),
            train_label=train_label.numpy(),
            val_pred=valid_pred.numpy(),
            val_label=valid_label.numpy(),
